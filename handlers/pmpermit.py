...
import helpers.mongo.pmpermitdb as CyberBytesCare
...
async def pmguard(client, message):
    ...
    if not arg:
        ...
        return
    await CyberBytesCare.set_limit(int(arg))
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await CyberBytesCare.set_permit_message(CyberBytesCare.PMPERMIT_MESSAGE)
        await message.edit("**Anti_PM message set to default**.")
        return
    await CyberBytesCare.set_permit_message(f"`{arg}`")
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await CyberBytesCare.set_block_message(CyberBytesCare.BLOCKED)
        await message.edit("**Block message set to default**.")
        return
    await CyberBytesCare.set_block_message(f"`{arg}`")
...
async def allow(client, message):
    ...
    pmpermit, pm_message, limit, block_message = await CyberBytesCare.get_pm_settings()
    await CyberBytesCare.allow_user(chat_id)
