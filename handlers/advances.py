...
CyberBytesCare = f"**꧁ 𒈞CybₑᵣByₜₑₛCₐᵣₑ ᵤₛₑᵣbₒₜ𒈞꧂**\n\n"
CyberBytesCare += f"━───────╯•╰───────━\n"
CyberBytesCare += f"➠ **ᴘʏᴛʜᴏɴ ᴠᴇʀsɪᴏɴ** : `3.10.5`\n"
CyberBytesCare += f"➠ **ᴘʏʀᴏɢʀᴀᴍ ᴠᴇʀsɪᴏɴ** : `{__version__}`\n"
CyberBytesCare += f"➠ **ᴠᴇʀsɪᴏɴ**  : `{2.0}`\n"
CyberBytesCare += f"➠ **ᴄʜᴀɴɴᴇʟ** : [❝𝐂𝐥𝐢𝐜𝐤❞](https://t.me/CyberBytesCare)\n"
CyberBytesCare += f"━───────╮•╭───────━\n\n"
CyberBytesCare += f"➠ **𒆜ʍǟӄɛ ʏօʊʀ օառ 𒆜:** [❝𝐂𝐥𝐢𝐜𝐤❞](https://gitHub.com/CyberBytesCare/Cyberbytescareuserbot)"
...
async def help(_, e: Message):
        CyberBytesCare = e.text.split(" ")
        if len(CyberBytesCare) > 1:
            helping = CyberBytesCare[1]
...
async def delayspam(xspam: Client, e: Message): 
    ...
    CyberBytesCare = kkk[1:]
    if len(CyberBytesCare) == 2:
       counts = int(CyberBytesCare[0])
       if int(e.chat.id) in GROUP:
            return await e.reply_text("**Sorry !! i Can't Spam Here.**")
       msg = str(CyberBytesCare[1])
       if re.search(Owners.lower(), msg.lower()):
            return await e.reply("**Sorry !!**")
       sleeptime = float(CyberBytesCare[0])
...
async def leasse(client: Client, message: Message):
    ...
    try:
        await client.leave_chat(CyberBytesCare)
...
async def raid(xspam: Client, e: Message):  
      CyberBytesCare = "".join(e.text.split(maxsplit=1)[1:]).split(" ", 2)
      if len(CyberBytesCare) == 2:
          counts = int(CyberBytesCare[0])
          if int(e.chat.id) in GROUP:
               return await e.reply_text("**Sorry !! i Can't Spam Here.**")
          ok = await xspam.get_users(CyberBytesCare[1])
          id = ok.id
#          try:
...
async def gban(app: Client, message):
    CyberBytesCare = await message.reply_text("**Processing**")
    ...
    if reply:
        ...
        if not user:
            await CyberBytesCare.edit("**Whome should I replyraid?**")
            return
    get_user = await app.get_users(user)
    mee= await app.get_me()
    if get_user.id == mee.id:
        await CyberBytesCare.edit("`Jaa Na Lawde Kahe Dimag Kha rha? Khudpe Raid kyu laga rha?`")
        return
    if int(get_user.id) in VERIFIED_USERS:
        await CyberBytesCare.edit("Chal Chal baap Ko mat sikha")
        return
    elif int(get_user.id) in SUDO_USERS:
        await CyberBytesCare.edit("Abe Lawde that guy part of my devs.")
        return
    await rraid_user(get_user.id)
    await CyberBytesCare.edit(f"**Successfully Reply Raid Started {get_user.first_name}!**")
